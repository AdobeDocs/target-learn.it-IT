...
# Configurare i rapporti A4T in [!DNL Analysis Workspace] per [!DNL Auto-Target] attività
...
>[!TIP]
>
>Se i modelli ML si stanno ottimizzando per una metrica attribuita in modo diverso da quella delle metriche visualizzate in un rapporto, le prestazioni dei modelli potrebbero non corrispondere alle attese. Per evitare questo problema, assicurati che le metriche di obiettivo nel rapporto utilizzino la stessa definizione e attribuzione di metrica utilizzata da [!DNL Target] modelli ML.
...
### conversioni definite dal target, oppure [!DNL Analytics] metriche con *Massimizzare il valore della metrica per visita*

Quando la metrica è un valore [!DNL Target] conversione o un [!DNL Analytics] metriche con **Massimizzare il valore della metrica per visita**, la definizione della metrica di obiettivo consente che più eventi di conversione si verifichino nella stessa visita.
...
1. Come prima, seleziona la **[!UICONTROL Componenti > Crea segmento]** opzione in [!DNL Analysis Workspace] barra degli strumenti.
2. Specifica un **[!UICONTROL Titolo]** per il segmento.

   Nell’esempio mostrato di seguito, il segmento è denominato [!DNL "Visits with an order"].
...
5. Utilizza il **[!UICONTROL è maggiore di]** e impostare il valore su 0.
...
Con le modifiche apportate al [!UICONTROL Visita] e metriche dell’obiettivo nelle sezioni precedenti, la modifica finale da apportare all’A4T predefinito per [!DNL Auto-Target] Il pannello di reporting consiste nel creare tassi di conversione che siano il rapporto corretto, ovvero quello della metrica di obiettivo corretta, rispetto a una metrica &quot;Visite&quot; filtrata in modo appropriato.
...
>[!TIP]
>
> Puoi anche creare questa metrica utilizzando [funzionalità metriche calcolate rapide](https://experienceleague.adobe.com/docs/analytics-learn/tutorials/components/calculated-metrics/quick-calculated-metrics-in-analysis-workspace.html).
...
*Figura 10: A4T finale [!UICONTROL Targeting automatico] rapporto in [!DNL Adobe Analytics] [!DNL Workspace], che combina tutte le regolazioni alle definizioni delle metriche descritte nelle sezioni precedenti di questa esercitazione.*
